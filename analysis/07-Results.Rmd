...
The summaries below correspond to the results of analyses outlined [here](https://github.com/LucianoRogerio/IITA_2022GS) and linked in the navbar above.
...
Summary of the number of unique plots, locations, years, etc. in the cleaned plot-basis data (`output/IITA_ExptDesignsDetected_2022Aug.rds`, download from FTP). See the data cleaning step here for details.

```{r Raw data}
library(tidyverse); library(magrittr); library(ragg)
rawdata<-readRDS(file=here::here("output","IITA_ExptDesignsDetected_2022Aug24.rds"))
rawdata %>% 
...
#tiff(filename = here::here("output", "Figures", "DensityplotPhenY.tiff"), res = 300, units = "cm",
#     compression = "lzw", height = 10, width = 22)
rawdata %>% 
...
There are 8459 genotyped-and-phenotyped clones, I could not find the clones with suffix `IITA-TMS-`.
...
Summarize the BLUPs from the training data, leveraging for each clone data across trials/locations without genomic information and to be used as input for genomic prediction downstream (`data/blups_forGP.rds`). See the mixed-model analysis step [here](02-GetBLUPS.html) and a subsequent processing step [here](04-PreprocessDataFiles.html) for details.
...
```{r}
#tiff(filename = here::here("output", "Figures", "DensityPlotBlupsY.tiff"), res = 300, units = "cm",
#     compression = "lzw", height = 10, width = 22)
blups %>% 
  select(Trait,blups) %>% 
  unnest(blups) %>% 
...
```{r}
#tiff(filename = here::here("output", "Figures", "BoxplotBlupsRel.tiff"), res = 300, units = "cm",
#     compression = "lzw", height = 10, width = 22)
blups %>% 
  select(Trait,blups) %>% 
  unnest(blups) %>% 
...
Summarize the marker data (`data/dosages_IITA_2022Aug31.rds`). See the pre-processing steps [here](04-PreprocessDataFiles.html).

```{r}
library(tidyverse); library(magrittr); library(CMplot)
snps<-readRDS(file=here::here("data","dosages_IITA_2022Aug31.rds"))
mrks<-colnames(snps) %>% 
  tibble(SNP_ID=.) %>% mutate(SNP = SNP_ID, .keep = "used") %>% 
  separate(SNP_ID,c("Chr","Pos","Allele"),"_") %>% 
  mutate(Chr=as.integer(gsub("S","",Chr)),
         Pos=as.numeric(Pos)) %>% select(SNP, Chr, Pos)
rm(snps)

#CMplot(mrks, plot.type = "d", bin.size = 1e6, col = c("darkgreen", "yellow", "red"),
#       file = "jpg", memo = "IITADensityMkrs", dpi = 500, file.output = T, verbose = TRUE)


```

![](../output/SNP-Density.Col1_IITADensityMkrs.Col0_IITADensityMkrs.jpg)
...
```{r}
ped %>% nrow(.) # 4579 pedigree entries
ped %>% 
...
821 families. Mean size 5.577, range 1-77.
...
```{r}
#tiff(filename = here::here("output", "Figures", "BoxPlotGpIS.tiff"), res = 300, units = "cm",
#     compression = "lzw", height = 10, width = 22)
accuracy %>% 
...
Based on the preliminary results (variance predictions not even done yet), the **reduced_set** (**LDprunedR2pt6**) suffers reduced prediction accuracy. I added a **medium_set** (**LDprunedR2pt8**) with about 13K SNP which produces GEBV/GETGV that are highly correlated with the full 30K SNP set and has the same accuracy.

Prediction accuracy estimates will be in `output/` ([here](ftp://ftp.cassavabase.org/marnin_datasets/IITA_2021GS/output/)) with filenames ending `_ClonePredAccuracy.rds` and `_CrossPredAccuracy.rds`.
...
saveRDS(cvResults, file = here::here("output", "oldResults", "stdcvResults2022.rds"))
```
...
```{r,fig.width = 8.5, fig.height=4, warning=F,message=FALSE}
cvMateSel2022 <- recalculated_accuracy %>% 
  filter(predOf=="Mean") %>% 
...
saveRDS(cvMateSel2022, file = here::here("output", "oldResults", "MateMeancvResults2022.rds"))
...
mutate <- dplyr::mutate
cvMateVarSel2022 <- recalculated_accuracy %>% 
  filter(predOf=="Var",
         Trait1==Trait2) %>% 
  mutate(Trait1=factor(Trait1,levels=c("SELIND",blups$Trait)),
         predOf=factor(paste0("Var",VarComp,"_",Filter),
                       levels=c("VarBV_33K","VarTGV_33K","VarBV_13K","VarTGV_13K","VarBV_8K","VarTGV_8K")))
saveRDS(cvMateVarSel2022, file = here::here("output", "oldResults", "MateVarcvResults2022.rds"))
...
Plot genomic prediction accuracy trends since 2019.

### Genomic Prediction accuracy trends in IITA

#### Get the accuracies from older predictions

##### 2022

This data was already been saved in the file `stdcvResults2022.rds` in [Standard cross-validation] section.
...
library(ggdist)
# PARENT-WISE CV RESULTS
accuracy_full<-readRDS(here::here("output", "oldResults","parentWiseCV_full_set_CrossPredAccuracy.rds"))
accuracy_medium<-readRDS(here::here("output", "oldResults","parentWiseCV_medium_set_CrossPredAccuracy.rds"))
accuracy_reduced<-readRDS(here::here("output", "oldResults","parentWiseCV_reduced_set_CrossPredAccuracy.rds"))
accuracy<-accuracy_medium$meanPredAccuracy %>%
  filter(Trait=="SELIND") %>%
  mutate(VarComp=gsub("Mean","",predOf),
         predOf="Mean",
...
standardCV_full<-readRDS(here::here("output", "oldResults",
                                    "standardCV_full_set_ClonePredAccuracy.rds"))
standardCV_medium<-readRDS(here::here("output", "oldResults",
                                    "standardCV_medium_set_ClonePredAccuracy.rds"))
standardCV_reduced<-readRDS(here::here("output", "oldResults",
                                       "standardCV_reduced_set_ClonePredAccuracy.rds"))
accuracy<-standardCV_full %>% 
  unnest(accuracyEstOut) %>% 
  select(-splits,-predVSobs,-seeds) %>% 
...
saveRDS(cvResults2021, file = here::here("output", "oldResults", "stdcvResults2021.rds"))
...
cvMateSel2021 <- recalculated_accuracy %>% 
  filter(predOf=="Mean") %>% 
  mutate(Trait1=factor(Trait1,levels=c("SELIND",blups$Trait)),
         predOf=factor(paste0("Mean",VarComp,"_",Filter),
                       levels=c("MeanBV_33K","MeanTGV_33K","MeanBV_13K","MeanTGV_13K","MeanBV_8K","MeanTGV_8K")))
saveRDS(cvMateSel2021, file = here::here("output", "oldResults", "MateMeancvResults2021.rds"))

cvMateVarSel2021 <- recalculated_accuracy %>% 
  filter(predOf=="Var") %>% 
  mutate(Trait1=factor(Trait1,levels=c("SELIND",blups$Trait)),
         predOf=factor(paste0("Var",VarComp,"_",Filter),
                       levels=c("VarBV_33K","VarTGV_33K","VarBV_13K","VarTGV_13K","VarBV_8K","VarTGV_8K")))
saveRDS(cvMateVarSel2021, file = here::here("output", "oldResults", "MateVarcvResults2021.rds"))
```
...
library(tidyverse)
library(magrittr)
cvresults <- readRDS(here::here("output", "oldResults", "cvresults_ModelA_chunk1.rds")) %>%
  bind_rows(readRDS(here::here("output", "oldResults", "cvresults_ModelA_chunk2.rds"))) %>%
  bind_rows(readRDS(here::here("output", "oldResults", "cvresults_ModelA_chunk3.rds"))) %>% 
  mutate(Model = "A") %>%
  bind_rows(readRDS(here::here("output", "oldResults", "cvresults_ModelADE_chunk1.rds")) %>% 
              bind_rows(readRDS(here::here("output", "oldResults", "cvresults_ModelADE_chunk2.rds"))) %>% 
              bind_rows(readRDS(here::here("output", "oldResults", "cvresults_ModelADE_chunk3.rds"))) %>% 
              mutate(Model = "ADE")) %>% filter(VersionOfBLUPs == "blups2stage") %>% 
  select(-VersionOfBLUPs) %>% mutate(Model = ifelse(Model == "A", "GEBV", "GETGV"))

saveRDS(cvresults, file = here::here("output", "oldResults", "stdcvResults2020.rds"))
```
...
library(magrittr); 
library(cowplot); 
files<-list.files(here::here("output", "oldResults", "CrossVal_72719/"))
pathway<-here::here("output", "oldResults", "CrossVal_72719/")
cv<-tibble(Files=files) %>% 
  dplyr::mutate(cvResults=map(Files,~readRDS(paste0(pathway,.))))
cv %<>% 
  ...
  mutate(Trait = gsub(x = Trait, pattern = "CrossVal_", replacement = ""))
saveRDS(cv, file = here::here("output", "oldResults", "stdcvResults2019.rds"))
```

#### Joint all the data for plotting

##### Standart Cross-Validation

```{r}
stdcv2019 <- readRDS(file = here::here("output", "oldResults", "stdcvResults2019.rds")) %>% 
  select(Trait, accuracy) %>% filter(Trait %in% c("DM", "HI", "logFYLD", "logRTNO",
                                                  "logTOPYLD", "MCMDS", "RTSZ", "TCHART")) %>%
  mutate(Model = "GEBV")

stdcv2020 <- readRDS(file = here::here("output", "oldResults", "stdcvResults2020.rds")) %>% 
  mutate(accuracy = ifelse(Model == "GEBV", accGEBV, accGETGV)) %>%
  select(Trait, accuracy, Model)

stdcv2021 <- readRDS(file = here::here("output", "oldResults", "stdcvResults2021.rds")) %>% 
  filter(Filter == "Full") %>%
  mutate(Model = gsub(x = predOf, pattern = "_Full", replacement = ""),
         accuracy = Accuracy) %>% 
  select(Trait, accuracy, Model) %>% filter(Trait %in% c("SELIND", "DM", "HI",
                                                         "logDYLD", "logFYLD", "logRTNO",
                                                  "logTOPYLD", "MCMDS", "BCHROMO"))
  stdcv2021$Trait <- droplevels(stdcv2021$Trait)

stdcv2022 <- readRDS(file = here::here("output", "oldResults", "stdcvResults2022.rds")) %>% 
  filter(Filter == "Full") %>%
  mutate(Model = gsub(x = predOf, pattern = "_Full", replacement = ""),
         accuracy = Accuracy) %>% 
  select(Trait, accuracy, Model) %>% filter(Trait %in% c("SELIND", "DM", "HI",
...
stdcvs <- stdcv2019 %>% mutate(Year = 2019) %>% 
  bind_rows(stdcv2020 %>% mutate(Year = 2020)) %>% 
  bind_rows(stdcv2021 %>% mutate(Year = 2021)) %>% 
  bind_rows(stdcv2022 %>% mutate(Year = 2022)) %>% filter(!Trait %in% c("RTSZ", "TCHART")) %>% 
  mutate(Trait = factor(Trait, levels = c("SELIND", "logFYLD", "logDYLD",
                                          "logRTNO", "DM", "MCMDS",
                                          "HI", "logTOPYLD", "BCHROMO")),
         Year = factor(Year, levels = c("2019", "2020", "2021", "2022")))
```

```{r}
tiff(filename = here::here("output", "Figures", "BoxPlotAccuracy4Years.tiff"),
     units = "cm", compression = "lzw", res = 300, width = 18, height = 12)
stdcvs %>% filter(Model == "GEBV") %>%  ggplot(aes(x = Year, y = accuracy, fill = Model)) +
  geom_boxplot() + facet_wrap(~Trait) + theme_bw() + theme(legend.position = "none",
                                                           plot.title = element_text(hjust = 0.5)) +
  labs(title = "IITA Genomic Prediction through time")
dev.off()
```
...
```{r}
MateMeancv2021 <- readRDS(file = here::here("output", "oldResults", "MateMeancvResults2021.rds")) %>%
  filter(Filter == "33K") %>% mutate(Trait = Trait1,
                                     accuracy = AccuracyEst,
                                     Model = ifelse(VarComp == "BV", "GEBV", "GETGV")) %>%
  select(Trait, accuracy, Model)

MateMeancv2022 <- readRDS(file = here::here("output", "oldResults", "MateMeancvResults2022.rds")) %>%
  filter(Filter == "33K") %>% mutate(Trait = Trait1,
                                     accuracy = AccuracyEst,
                                     Model = ifelse(VarComp == "BV", "GEBV", "GETGV")) %>%
  select(Trait, accuracy, Model)
...
MateMeancvs <- MateMeancv2021 %>% mutate(Year = 2021) %>% 
  bind_rows(MateMeancv2022 %>% mutate(Year = 2022)) %>% 
  mutate(Year = factor(Year, levels = c("2021", "2022")))

MateVarcv2021 <- readRDS(file = here::here("output", "oldResults", "MateVarcvResults2021.rds")) %>%
  filter(Filter == "33K", Trait1 == Trait2) %>% mutate(Trait = Trait1,
                                     accuracy = AccuracyEst,
                                     Model = ifelse(VarComp == "BV", "GEBV", "GETGV")) %>%
  select(Trait, accuracy, Model)

MateVarcv2022 <- readRDS(file = here::here("output", "oldResults", "MateVarcvResults2022.rds")) %>%
  filter(Filter == "33K", Trait1 == Trait2) %>% mutate(Trait = Trait1,
                                     accuracy = AccuracyEst,
                                     Model = ifelse(VarComp == "BV", "GEBV", "GETGV")) %>%
  select(Trait, accuracy, Model)
...
MateVarcvs <- MateVarcv2021 %>% mutate(Year = 2021) %>% 
  bind_rows(MateVarcv2022 %>% mutate(Year = 2022)) %>% 
  mutate(Year = factor(Year, levels = c("2021", "2022")))
```

```{r}
#tiff(filename = here::here("output", "Figures", "GenMateMean4Years.tiff"), res = 300,
#     units = "cm", compression = "lzw", width = 18, height = 12)
MateMeancvs %>% ggplot(aes(x = Year, y = accuracy, fill = Model)) +
  geom_boxplot() + theme_bw() + facet_wrap(~Trait) +
  ylim(0,1) +
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5)) +
  labs(title = "IITA Genomic Mate selection through time\nMean")
#dev.off()
```

```{r}
#tiff(filename = here::here("output", "Figures", "GenMateVar4Years.tiff"), res = 300,
#     units = "cm", compression = "lzw", width = 18, height = 12)
MateVarcvs %>% ggplot(aes(x = Year, y = accuracy, fill = Model)) + 
  geom_boxplot() + facet_wrap(~Trait) + theme(legend.position = "none",
                                                           plot.title = element_text(hjust = 0.5)) +
  labs(title = "IITA Genomic Mate selection through time\nVar")
#dev.off()
```
...
rawdata<-readRDS(here::here("output","IITA_ExptDesignsDetected_2022Aug24.rds"))
for_trend_plot<-si_getgvs %>% 
  left_join(rawdata %>% 
              distinct(germplasmName,GID), by = c("GID", "FullSample")) %>% 
  group_by(GID) %>% 
  slice(1) %>% 
  ungroup()
...
for_trend_plot %<>% 
  ...
  mutate(Year_Accession=case_when(grepl("2013_|TMS13",germplasmName)~2013,
                                  ...
                                  grepl("TMS21",germplasmName)~2021,
                                  !grepl("2013_|TMS13|TMS14|TMS15|TMS16|TMS17|TMS18|TMS19|TMS20|TMS21",germplasmName)~Year_Accession))
...
si_getgvs %>% 
  mutate(Cohort=NA,
         Cohort=ifelse(grepl("TMS21",GID,ignore.case = T),"TMS21",
                       ifelse(grepl("TMS20",GID,ignore.case = T),"TMS20",
         ...
         Cohort=factor(Cohort,levels = c("PreGS","TMS13","TMS14","TMS15","TMS16",
                                         "TMS17","TMS18","TMS19","TMS20", "TMS21"))) %>% 
...
parentsToPredictCrosses<-readRDS(file = here::here("output",
                                                   "parentsToPredictCrosses_2022Set05.rds"))
for_selected_plot<-si_getgvs %>% 
  mutate(Cohort=NA,
         ...
         Cohort=factor(Cohort,levels = c("PreGS","TMS13","TMS14","TMS15","TMS16","TMS17","TMS18","TMS19","TMS20", "TMS21")))
for_selected_plot %>% 
...
```{r, eval=T}
library(tidyverse); library(magrittr); library(ggdist)
crossPreds<-read.csv(here::here("output","genomicMatePredictions_2022Set05.csv"), stringsAsFactors = F, header = T)
...
cross_group_order<-crossPreds %>% 
  ...
  mutate(sireGroup=factor(sireGroup,levels=c("PreGS","TMS13","TMS14","TMS15","TMS16",
                                             "TMS17","TMS18","TMS19","TMS20","TMS21")),
         damGroup=factor(damGroup,levels=c("PreGS","TMS13","TMS14","TMS15","TMS16",
                                           "TMS17","TMS18","TMS19","TMS20","TMS21"))) %>% 
...
-   [**Download the CSV for the top 50 crosses and their selection index means, SDs and UC here**](ftp://ftp.cassavabase.org/marnin_datasets/IITA_2021GS/output/top50crosses_2022Sep06.csv) or
-   [**Download the complete CSV of ALL genomic mate predictions here**](ftp://ftp.cassavabase.org/marnin_datasets/IITA_2021GS/output/genomicMatePredictions_2022Sep05.csv): for all crosses and both SELIND and component trait predictions.
...
top50crosses<-forplot %>% 
  ...
  ungroup()
top50crosses %>% 
  write.csv(.,file = here::here("output","top50crosses_2022Sep06.csv"), row.names = F)
top50crosses %>% 
  rmarkdown::paged_table()
```
